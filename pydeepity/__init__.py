...
class SimplePCN(dy.SimplePCNetwork, _PCNMixin):
    ...
    def fit_iavg(
        ...
        """
        Specialized training loop for deep networks that utilizes Class-Average Caching (I_avg).
        It manages the StreamAlignedBatcher and dynamically seeds the hidden layer.
        """
        console = Console()
        bsz = self.batch_size
        
        # Native C++ Batcher handles class-alignment seamlessly
        ...
        console.print(
            ...
            f"{steps} inference steps [dim]|[/dim] {n_batches} batches/epoch\n"
        )
        ...
        with progress:
            ...
            for epoch in range(epochs):
                ...
                for b in range(n_batches):
                    ...
                    # 1. Apply cache to hidden layer
                    if cache:
                        init_beliefs = np.zeros((bsz, hidden_size), dtype=np.float32)
                    ...
                    # 3. Cache the newly settled beliefs
                    settled = np.array(self.layers[hidden_layer_index].beliefs, copy=False).reshape(bsz, hidden_size)
                    for c in range(num_classes):
