...
#pragma region tanh
    inline void tanh(float *RESTRICT x, const size_t n) noexcept
    {
        ...
        const __m512 tiny = _mm512_set1_ps(TANH_TINYLIMIT);
        const __m512 poly_limit = _mm512_set1_ps(TANH_POLY_LIMIT);
        const __m512 big = _mm512_set1_ps(TANH_BIGLIMIT);

        const __m512 one = _mm512_set1_ps(1.0f);
        const __m512 zero = _mm512_setzero_ps();

        const __m512 p0 = _mm512_set1_ps(P0);
        const __m512 p1 = _mm512_set1_ps(P1);
        const __m512 p2 = _mm512_set1_ps(P2);

        const __m512 q0 = _mm512_set1_ps(Q0);
        const __m512 q1 = _mm512_set1_ps(Q1);
        const __m512 q2 = _mm512_set1_ps(Q2);
...
#pragma omp parallel for schedule(static) if (n > 65536)
        for (size_t i = 0; i < simd_end; i += 16)
        {
            ...
            __m512 ax = _mm512_and_ps(x0, abs_mask);

            __mmask16 big_mask = _mm512_cmp_ps_mask(ax, big, _CMP_GE_OQ);
            __mmask16 poly_mask = _mm512_cmp_ps_mask(ax, poly_limit, _CMP_LT_OQ);
            __mmask16 tiny_mask = _mm512_cmp_ps_mask(ax, tiny, _CMP_LT_OQ);

            // Polynomial path
            __m512 s = _mm512_mul_ps(x0, x0);

            __m512 p = _mm512_fmadd_ps(p0, s, p1);
            p = _mm512_fmadd_ps(p, s, p2);

            __m512 q = _mm512_add_ps(s, q0);
            q = _mm512_fmadd_ps(q, s, q1);
            q = _mm512_fmadd_ps(q, s, q2);

            __m512 poly = _mm512_add_ps(
                x0, _mm512_mul_ps(_mm512_mul_ps(x0, s), _mm512_div_ps(p, q)));

            // Exponential path
            alignas(64) float exp_input[16];
            alignas(64) float exp_output[16];
            alignas(64) float values[16];

            _mm512_store_ps(values, ax);
            bool need_exp = false;

            for (int j = 0; j < 16; j++)
            {
                // Only calculate exp if it's NOT big and NOT poly
                if (!((big_mask >> j) & 1) && !((poly_mask >> j) & 1))
                {
            ...
            if (need_exp)
            {
                ...
                for (int j = 0; j < 16; j++)
                {
                    exp_output[j] = 1.0f - 2.0f / (exp_input[j] + 1.0f);
                }
            }

            __m512 exp_result = _mm512_load_ps(exp_output);

            // Restore sign
            exp_result = _mm512_or_ps(
                _mm512_and_ps(x0, sign_mask), _mm512_and_ps(exp_result, abs_mask));

            // Select result via sequential overlay
            __m512 result = exp_result;
            result = _mm512_mask_blend_ps(poly_mask, result, poly);
            result = _mm512_mask_blend_ps(big_mask, result, _mm512_or_ps(_mm512_and_ps(x0, sign_mask), one));
            result = _mm512_mask_blend_ps(tiny_mask, result, x0);
...
#pragma omp parallel for schedule(static) if (n > 65536)
        for (size_t i = 0; i < simd_end; i += 8)
        {
            ...
            __m256 big_mask = _mm256_cmp_ps(ax, big, _CMP_GE_OQ);
            __m256 poly_mask = _mm256_cmp_ps(ax, poly_limit, _CMP_LT_OQ);
            __m256 tiny_mask = _mm256_cmp_ps(ax, tiny, _CMP_LT_OQ);
...
#else
            __m256 p = _mm256_add_ps(_mm256_mul_ps(p0, s), p1);
            p = _mm256_add_ps(_mm256_mul_ps(p, s), p2);

            __m256 q = _mm256_add_ps(s, q0);
            q = _mm256_add_ps(_mm256_mul_ps(q, s), q1);
            q = _mm256_add_ps(_mm256_mul_ps(q, s), q2);
#endif

            __m256 poly = _mm256_add_ps(
                x0, _mm256_mul_ps(_mm256_mul_ps(x0, s), _mm256_div_ps(p, q)));
            ...
            for (int j = 0; j < 8; j++)
            {
                bool is_big = reinterpret_cast<uint32_t *>(&big_mask)[j];
                bool is_poly = reinterpret_cast<uint32_t *>(&poly_mask)[j];
            ...
            __m256 exp_result = _mm256_load_ps(exp_output);

            // Restore sign
            exp_result = _mm256_or_ps(
                _mm256_and_ps(x0, sign_mask), _mm256_and_ps(exp_result, abs_mask));

            // Select result via sequential overlay
            __m256 result = exp_result;
            result = _mm256_blendv_ps(result, poly, poly_mask);
            result = _mm256_blendv_ps(result, _mm256_or_ps(_mm256_and_ps(x0, sign_mask), one), big_mask);
            result = _mm256_blendv_ps(result, x0, tiny_mask);
        ...
        const __m128 tiny = _mm_set1_ps(TANH_TINYLIMIT);
        const __m128 poly_limit = _mm_set1_ps(TANH_POLY_LIMIT);
        const __m128 big = _mm_set1_ps(TANH_BIGLIMIT);

        const __m128 one = _mm_set1_ps(1.0f);
        const __m128 zero = _mm_setzero_ps();

        const __m128 p0 = _mm_set1_ps(P0);
        const __m128 p1 = _mm_set1_ps(P1);
        const __m128 p2 = _mm_set1_ps(P2);

        const __m128 q0 = _mm_set1_ps(Q0);
        const __m128 q1 = _mm_set1_ps(Q1);
        const __m128 q2 = _mm_set1_ps(Q2);
...
#pragma omp parallel for schedule(static) if (n > 65536)
        for (size_t i = 0; i < simd_end; i += 4)
        {
            ...
            __m128 ax = _mm_and_ps(x0, abs_mask);

            __m128 big_mask = _mm_cmpge_ps(ax, big);
            __m128 poly_mask = _mm_cmplt_ps(ax, poly_limit);
            __m128 tiny_mask = _mm_cmplt_ps(ax, tiny);

            // Polynomial path
            __m128 s = _mm_mul_ps(x0, x0);

#ifdef __FMA__
            __m128 p = _mm_fmadd_ps(p0, s, p1);
            p = _mm_fmadd_ps(p, s, p2);

            __m128 q = _mm_add_ps(s, q0);
            q = _mm_fmadd_ps(q, s, q1);
            q = _mm_fmadd_ps(q, s, q2);
#else
            __m128 p = _mm_add_ps(_mm_mul_ps(p0, s), p1);
            p = _mm_add_ps(_mm_mul_ps(p, s), p2);

            __m128 q = _mm_add_ps(s, q0);
            q = _mm_add_ps(_mm_mul_ps(q, s), q1);
            q = _mm_add_ps(_mm_mul_ps(q, s), q2);
#endif

            __m128 poly = _mm_add_ps(
                x0, _mm_mul_ps(_mm_mul_ps(x0, s), _mm_div_ps(p, q)));

            // Exponential path
            alignas(16) float exp_input[4];
            alignas(16) float exp_output[4];
            alignas(16) float values[4];

            _mm_store_ps(values, ax);
            bool need_exp = false;
            ...
            __m128 exp_result = _mm_load_ps(exp_output);

            // Restore sign
            exp_result = _mm_or_ps(
                _mm_and_ps(x0, sign_mask), _mm_and_ps(exp_result, abs_mask));

            // Select result using bitwise blend for broad SSE compatibility
            // blend(a, b, mask) -> (a & ~mask) | (b & mask)
            __m128 result = exp_result;

            result = _mm_or_ps(_mm_andnot_ps(poly_mask, result), _mm_and_ps(poly_mask, poly));

            __m128 big_val = _mm_or_ps(_mm_and_ps(x0, sign_mask), one);
            result = _mm_or_ps(_mm_andnot_ps(big_mask, result), _mm_and_ps(big_mask, big_val));

            result = _mm_or_ps(_mm_andnot_ps(tiny_mask, result), _mm_and_ps(tiny_mask, x0));
        ...
        for (size_t i = simd_end; i < n; i++)
        {
            ...
            {
                float s = v * v;
                x[i] = v + v * s * (((P0 * s + P1) * s + P2) / (((s + Q0) * s + Q1) * s + Q2));
            }
            else
            {
                float e = Sleef_expf_u10(2.0f * av);
                x[i] = copysignf(1.0f - 2.0f / (e + 1.0f), v);
            }
        }
    }
