...
void bind_layers(nb::module_& m)
{
  ...
  // --- Add near the top of pybinding.cpp, alongside the other
  // #include <deepity/networks/...> lines ---

#include <deepity/networks/FullPCNetwork.h>

  // --- LAYER BINDING: add right after the existing dkpLayerCls block
  // (after line 307's closing semicolon), inside bind_layers(). Direct
  // mirror of DirectKPPCLayer's binding -- same BindCommonPCLayer<>
  // reuse, same DKP-specific extras, plus set_mu_pc_scale/set_residual
  // and their read-only counterparts. ---
