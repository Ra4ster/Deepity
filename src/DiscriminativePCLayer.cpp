...
#pragma omp for
    ...
    {
        PCN_TIME(LayerProfile(this).calculateStateTotal);
        const size_t N = (size_t)batchSize * size;

        {
            PCN_TIME(LayerProfile(this).errorConstruction);
        ...
        {
            PCN_TIME(LayerProfile(this).energyCalculation);
#pragma omp parallel for schedule(static) reduction(+ : totalEnergy)
        ...
        if (nextSize > 0)
        {
            ...
            {
                PCN_TIME(LayerProfile(this).forwardGemm);
                cblas_sgemm(
                    CblasRowMajor,
                    CblasNoTrans,
                    CblasTrans,
            ...
            {
                PCN_TIME(LayerProfile(this).biasAdd);
                for (int batch = 0; batch < batchSize; ++batch)
            ...
            {
                PCN_TIME(LayerProfile(this).activationForward);
                activation(mu, Nout);
            }
        }

        LayerProfile(this).calculateStateCalls++;
        return totalEnergy;
    }

    void DiscriminativePCLayer::UpdateState() noexcept
    {
        PCN_TIME(LayerProfile(this).updateStateTotal);
        size_t N = (size_t)batchSize * size;

        if (nextSize > 0)
        {
            size_t Nout = (size_t)batchSize * nextSize;
            PCN_TIME(LayerProfile(this).activationDerivative);
            activationDerivative(mu, Nout, true); // raw activation -> f'(), in place
        }

        if (isClamped)
        {
            LayerProfile(this).updateStateCalls++;
            return;
        }

        {
            PCN_TIME(LayerProfile(this).localErrorTerm);
#pragma omp parallel for schedule(static)
        ...
        if (layerAbove != nullptr && nextSize > 0)
        {
            ...
            {
                PCN_TIME(LayerProfile(this).bottomUpConstruction);
#pragma omp parallel for schedule(static)
                for (int batch = 0; batch < batchSize; ++batch)
                {
            ...
            {
                PCN_TIME(LayerProfile(this).feedbackGemm);
        ...
        {
            PCN_TIME(LayerProfile(this).saxpyUpdate);
    ...
    {
        PCN_TIME(LayerProfile(this).updateWeightsTotal);
        if (layerAbove == nullptr || nextSize == 0)
            return;
...
#pragma omp parallel for schedule(static)
    ...
    {
        PCN_TIME(LayerProfile(this).updatePrecisionTotal);
        if (layerBelow == nullptr)
            return;
