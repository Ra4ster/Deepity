    ...
    public:
        ...
        void Test1_InferenceConvergence()
        {
            ...
            for (int i = 0; i < 100; ++i)
            {
                ...
                if (i > 0 && energy > prevEnergy + 1e-4f)
                    passed = false;
                prevEnergy = energy;
            }
        ...
        void Test2_GradientCheckWeights()
        {
            ...
            for (int i = 0; i < 2 * 3; ++i)
            {
                ...
                if (err > 1e-3f && std::abs(numGrad) > 1e-5f)
                    passed = false;
            }
        ...
        void Test9_BufferPoisoning()
        {
            ...
            if (layer.nextSize > 0)
            {
                ...
                {
                    if (std::isnan(layer.mu[i]))
                        passed = false;
                    if (std::isnan(layer.bottom_up[i]))
                        passed = false;
                }
            }
